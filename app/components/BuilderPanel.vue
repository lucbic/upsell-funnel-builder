...
onConnect((connection: Connection) => {
  if (!store.validateConnection(connection)) {
    toast.add({
      ...
      description: 'This connection is not allowed by the funnel rules.',
...
const onDrop = (event: DragEvent) => {
  const nodeType = event.dataTransfer?.getData('application/vueflow') as Funnel.NodeType
  if (!nodeType) return
...
<template>
  ...
  >
    <template #node-sales-page="props">
      <NodesSalesPageNode v-bind="props" />
    </template>

    <template #node-order-page="props">
      <NodesOrderPageNode v-bind="props" />
    </template>

    <template #node-upsell="props">
      <NodesUpsellNode v-bind="props" />
    </template>

    <template #node-downsell="props">
      <NodesDownsellNode v-bind="props" />
    </template>

    <template #node-thank-you="props">
      <NodesThankYouNode v-bind="props" />
    </template>
